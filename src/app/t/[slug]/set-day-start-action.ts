...
import { setDayStart } from "@/server/trips/set-day-start";

export interface SetDayStartState {
  readonly error: string | null;
}

const setDayStartSchema = z.object({
  slug: z.string().min(1).max(80),
  editKey: z.string().regex(EDIT_KEY_PATTERN),
  dayId: z.string().min(1).max(40),
  startAtMinutes: z.number().int().min(0).max(MINUTES_PER_DAY - 1),
});
 ...
 * yours are deliberately given the same sentence: telling them apart turns this
 * into a way to test slugs.
 */
export async function setDayStartAction(input: unknown): Promise<SetDayStartState> {
  const parsed = setDayStartSchema.safeParse(input);
  if (!parsed.success) {
    return { error: "That time could not be read. Choose it again." };
  }

  const { editKey, ...rest } = parsed.data;
  const result = await setDayStart(
